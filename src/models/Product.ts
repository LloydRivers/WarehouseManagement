...
export abstract class Product {
  ...
  updateStock(amount: number): void {
    ...
    if (newStock < 0) {
      throw new InvalidEmailError("Stock cannot be negative");
    }
  ...
  addSupplier(supplier: Supplier, price: number): void {
    if (this.suppliers.some((s) => s.supplier.id === supplier.id)) {
      throw new InvalidEmailError("Supplier already exists for this product");
    }
  ...
  removeSupplier(supplierId: string): void {
    ...
    if (index !== -1) {
      ...
      throw new InvalidEmailError("Supplier not found");
    }
  }
